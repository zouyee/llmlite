...
pub const UnifiedHandler = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, savings_store: *savings_store_mod.SavingsStore) UnifiedHandler {
        return .{
            .allocator = allocator,
    ...
    pub fn handleGet(self: *UnifiedHandler, connection: std.net.Server.Connection, request_text: []const u8) !void {
        ...
        const json_response = shared.serializeUnifiedResponse(self.allocator, response_data) catch {
            try writeJsonResponse(connection, 500, "{\"error\":\"Failed to serialize response\"}");
            return;
        };
        defer self.allocator.free(json_response);
        try writeJsonResponse(connection, 200, json_response);
    }
};

fn parseQueryParam(request_text: []const u8, prefix: []const u8) ?[]const u8 {
    const path_end = std.mem.indexOf(u8, request_text, " HTTP/1.") orelse return null;
